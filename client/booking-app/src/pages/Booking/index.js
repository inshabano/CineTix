...
function Booking() {
  ...
  const generateSeatsLayout = useCallback(
    (totalSeats, bookedSeatsList = []) => {
  ...
  const fetchShowData = useCallback(async () => {
    ...
    try {
      ...
      if (showResponse.success) {
        ...
        const initialSeats = generateSeatsLayout(
          showResponse.data.totalSeats || 150,
          showResponse.data.bookedSeats || []
        );
  ...
  const handleProceedToPay = async () => {
    ...
    try {
      ...
      if (!orderResponse.success) {
        message.error({
          content: orderResponse.message || "Failed to create payment order.",
          key: "payment_init",
          duration: 3,
        });
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          if (verifyData.success) {
            message.success({
              content: "Payment verification successful. Confirming booking...",
              key: "payment_verify",
              duration: 2,
            });
            ...
            if (finalBookingResponse.success) {
              message.success({
                content: "Booking confirmed! Email sent.",
                key: "payment_verify",
                duration: 3,
              });
              setTimeout(() => {
              ...
              message.error({
                content:
                  finalBookingResponse.message || "Failed to finalize booking.",
                key: "payment_verify",
                duration: 5,
              });
              console.error(
                "Final booking creation failed:",
                finalBookingResponse
              );
            }
          } else {
            message.error({
              content: verifyData.message || "Payment verification failed.",
              key: "payment_verify",
              duration: 5,
            });
      ...
      rzp.on("payment.failed", function (response) {
        message.error({
          content: `Payment Failed: ${response.error.description}`,
          key: "payment_init",
          duration: 5,
        });
            ...
            <Col>
              <Card
                title={
                  <div>
                    <h1>
                      {showDetails.movie.movieName}{" "}
                      <span>({showDetails.movie.language})</span>
                    </h1>
                    ...
                    <p>
                      ...
                      {showDetails.totalSeats -
                        (showDetails.bookedSeats
                          ? showDetails.bookedSeats.length
                          : 0)}
                    </p>
                  </div>
                }
              >
                    ...
                    <div key={rowIndex} className={styles.seatRowContainer}>
                      ...
                      {seatRow.map((seat) => {
                        ...
                        return (
                          <div
                            key={seat.id}
                            className={`${styles.seatUnit} ${
                              styles[seat.status]
                            } ${
                              isCurrentlySelected ? styles.selectedSeat : ""
                            }`}
                ...
                <div className={styles.seatStatusLegend}>
                  <div className={styles.legendItemEntry}>
                    <span
                      className={`${styles.seatUnit} ${styles.available}`}
                    ></span>{" "}
                    Available
                  </div>
                  <div className={styles.legendItemEntry}>
                    <span
                      className={`${styles.seatUnit} ${styles.selectedSeat}`}
                    ></span>{" "}
                    Selected
                  </div>
                  <div className={styles.legendItemEntry}>
                    <span
                      className={`${styles.seatUnit} ${styles.booked}`}
                    ></span>{" "}
                    Booked
                  </div>
                </div>
