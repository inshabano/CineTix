...
const MovieDetail = () => {
    ...
    const fetchMovieDetails = async () => {
      ...
      try {
        ...
        if (watchlistResponse.success && watchlistResponse.data) {
          ...
          const movieIsCurrentlyInWatchlist =
            watchlistMovieIds.includes(movieid);
          setIsInWatchlist(movieIsCurrentlyInWatchlist);
          console.log(
            `Movie ID "${movieid}" in fetched watchlist: ${movieIsCurrentlyInWatchlist}`
          );
        } else if (
          !watchlistResponse.success &&
          watchlistResponse.message === "User not authenticated."
        ) {
          console.log("User not authenticated. Heart defaults to white.");
          setIsInWatchlist(false);
        } else {
          console.error(
            "Failed to fetch watchlist with error:",
            watchlistResponse.message
          );
          message.error(
            watchlistResponse.message || "Could not retrieve watchlist status."
          );
          setIsInWatchlist(false);
        }
      } catch (error) {
  ...
  const directorName =
    movie.director && movie.director.name ? movie.director.name : "N/A";
          ...
          <div className={styles.details}>
            <div className={styles.titleAndWatchlist}>
              ...
              <div
                id="heart"
                ref={heartButtonRef}
                className={`${styles.watchlistHeartButton} ${
                  isInWatchlist ? styles.activeHeart : ""
                }`}
                  ...
                  (day) => {
                    ...
                    return (
                      <span
                        ...
                        onClick={() =>
                          isSelectable && handleDateSelect(dateObj)
                        }
                      >
                        {day}
                      </span>
