    ...
    const fetchUserProfileData = async () => {
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                if (response.statusCode === 401 || response.message === "User not authenticated.") {
                    navigate('/login');
                }
            }
        } catch (err) {
        ...
        return (
            <div className={styles.container}>
                <div className={styles.emptyState}>
                    ...
                    <button className={styles.exploreButton} onClick={() => navigate('/login')}>Go to Login</button>
                </div>
            </div>
        );
    }
                    ...
                    <div className={styles.infoBlock}>
                        ...
                        {profileData.memberSince && (
                            <div className={styles.detailRow}>
                                ...
                                <span className={styles.detailValue}>{moment(profileData.memberSince).format('MMM DD, YYYY')}</span>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        {!isEditing ? (
                            <>
                                <div className={styles.detailRow}>
                                    ...
                                    <span className={styles.detailValue}>{profileData.fullName || 'N/A'}</span>
                                </div>
                                <div className={styles.detailRow}>
                                    <span className={styles.detailLabel}>Mobile:</span>
                                    <span className={styles.detailValue}>{profileData.mobileNumber || 'N/A'}</span>
                                </div>
                ...
                <div className={styles.card}> 
                    <div className={styles.infoBlock}>
                        <div className={styles.activityLinksContainer}>
                            <button className={styles.activityLink} onClick={() => navigate('/mybookings')}>My Bookings</button>
                            <button className={styles.activityLink} onClick={() => navigate('/my-watchlist')}>My Watchlist</button>
                        </div>
                    </div>
                </div>
            </div>
