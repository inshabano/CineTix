...
import { getWatchlist, removeMovieFromWatchlist } from '../../services/watchlist';
...
const WatchlistPage = () => {
    ...
    return (
        <div className={styles.container}>
            <h1>My Watchlist</h1>
            {watchlist.length === 0 ? (
                <p className={styles.emptyWatchlist}>Your watchlist is empty. Start adding some movies!</p>
            ) : (
                <div className={styles.movieGrid}>
                    {watchlist.map(movie => (
                        <div key={movie._id} className={styles.movieCard}>
                            ...
                            <p className={styles.movieGenre}>{Array.isArray(movie.genre) ? movie.genre.join(', ') : movie.genre}</p>
                            <button 
