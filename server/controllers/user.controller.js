...
const onRegister = async (req, res) => {
  ...
  if (!username || !email || !password) {
    return res
      .status(400)
      .send({ success: false, message: "Missing some feilds" });
  }
  try {
    ...
    return res
      .status(201)
      .send({ success: true, message: "Registration completed! Please login" });
  } catch (err) {
    return res
      .status(500)
...
const getCurrentUser = async (req, res) => {
  ...
  jwt.verify(token, process.env.SECRET_KEY, async (err, payload) => {
    if (err) {
      return res
        .status(403)
        .send({ message: "You are not allowed to access! Invalid Token" });
    }
