  ...
  try {
    ...
    if (!updatedUser) {
      ...
      if (userExists) {
        return res
          .status(200)
          .json({ success: true, message: "Movie already in watchlist." });
      } else {
        return res
          .status(404)
    ...
    res.status(500).json({
      success: false,
      message: "Server error: Could not add movie to watchlist.",
    });
  }
};
  ...
  try {
    ...
    if (!movieId) {
      return res.status(400).json({
        success: false,
        message: "Movie ID is required in parameters.",
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Server error: Could not remove movie from watchlist.",
    });
  }
};
...
const getWatchlist = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error: Could not retrieve watchlist.",
    });
  }
};
